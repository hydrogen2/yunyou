...
- [ ] ONE English track, and it is clear English: `narration.script` is written in plain words and short sentences for a
      non-native reader. There is no second register — `narration.variants`, `after_script_variants` and
      `waypoint_script_variants` were retired on 2026-09-03 (DECISIONS.md D8: the founder is the audience and cannot
      judge a literary register, so prose in it is prose nobody reviews). Do not re-add them; the validator warns and
      neither the player nor the renderer reads them. Other languages are whole-file locale overlays
      (`i18n/<code>.json`, template `i18n-locale.md`), never variants
- [ ] the same plain register holds for every spoken key in the scene — `script`, `after_script` and (walk scenes)
      `waypoint_script` — so the voice never changes register mid-scene (fact-check A5). Persona-owned lines (R4) keep
      their own wording
- [ ] a qualifier travels with the sentence it qualifies ("or part of a mile", "since 1982",
